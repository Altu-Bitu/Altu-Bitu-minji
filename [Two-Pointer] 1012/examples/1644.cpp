...
//         if (sum==target) { // 구간의 합이 target과 동일할 경우
//             ans++;
//             left++; right++; // 모든 포인터 이동
//             if (right == size)
//                 break;
//             sum -= prime_list[left-1]; // 이번 구간에 제외되는 값(이전 left값)
//             sum += prime_list[right]; // 이번 구간에 추가되는 값
//         }
//         else if (sum > target){ // 구간의 합이 target보다 클 때
//             left++; // 합을 작게 하기 위해 left를 이동
//             sum -= prime_list[left-1];
//         }
//         else if (sum < target){ // 구간의 합이 target보다 작을 때
//             right++; // 합을 크게 하기 위해 right를 이동
//             if (right == size)
//                 break;
//             sum += prime_list[right];
//         }

         if (sum >= target){ // 구간의 합이 target보다 크거나 같을 때
             if (sum == target) // 같다면
                 ans++;
             left++; // 합을 작게 하기 위해 left를 이동
             sum -= prime_list[left-1];
