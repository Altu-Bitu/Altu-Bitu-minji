...
vector<int> gemListTwoPointer(vector<string> &gems, int gem_size){
    ...
    while(left<=right && right < size){
        if (m.size() == gem_size){ // 모든 종류의 보석이 있음
            ...
            m[gems[left-1]]--;
            if (m[gems[left-1]]==0) // 더 이상 이 보석이 없다면 제거
                m.erase(gems[left-1]);
        }
        else if (m.size() < gems.size()){ // 모든 종류의 보석이 담기지 않음
...
vector<int> solution(vector<string> gems){
    ...
    int gem_size = unique_gem.size();
    answer = gemList(gems, gem_size);
